...
type ``Nip27 Mentions``(output:ITestOutputHelper) =
    ...
    let ``Parse npub reference`` () =
        ...
        |> should equal  [Tag ("p", ["9e30e940238cd9ebebc6328176dd4d109812129442f2a6c38727fc66fa7ea90a"])]
    ...
    let ``Parse multiple npub references`` () =
        ...
        |> Content.extractReferences
        |> should equal [
            Tag ("p", ["9e30e940238cd9ebebc6328176dd4d109812129442f2a6c38727fc66fa7ea90a"])
            Tag ("p", ["a8bf32d90f21bc06cd5631c2a1144d18a3c926e436badce43b943e53a25f4ca9"])
        ]
...
type ``Simple references``(output:ITestOutputHelper) =
    ...
    let ``Parse hashtag reference`` () =
        "Hello #nostr world"
        |> Content.extractReferences
        |> should equal  [Tag ("t", ["nostr"])]

    [<Fact>]
    let ``Parse web reference reference`` () =
        "My repo https://github.com/lontivero/Nostra"
        |> Content.extractReferences
        |> should equal  [Tag ("r", ["https://github.com/lontivero/Nostra"])]
...
type ``Composed references``(output:ITestOutputHelper) =
    ...
    let ``Parse all kind of references/mentions`` () =
        ...
        |> should equal [
           Tag ("t", ["nostr"])
           Tag ("r", ["https://github.com/lontivero/Nostra"])
           Tag ("e", ["e6c2eb23026b273fd887bb17968d19b7fb4aded740b73a7bff282e0e1355ea5f"; "wss://nos.lol"; "wss://nostr-pub.wellorder.net"])]