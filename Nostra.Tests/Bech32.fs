...
type ``Nip19 Bech32-Shareable entities``(output:ITestOutputHelper) =
    ...
    let ``Encode/Decode npub`` () = 
        let secKey = Key.createNewRandom()
        let pubkey = Key.getPubKey secKey
        NPub pubkey
        |> encodeDecode
        |> function
        | NPub decodedPubKey ->
            should equal (decodedPubKey.ToBytes()) (pubkey.ToBytes())
    ...
    let ``Encode/Decode nprofile`` () =
        let nprofile =
            NProfile(
                ECXOnlyPubKey.Create (Utils.fromHex "3bf0c63fcb93463407af97a5e5ee64fa883d107ef9e558472c4eb9aaaefa459d"), [
                    "wss://r.x.com"
                    "wss://djbas.sadkb.com"
                ]) 
    ...
    let ``Encode/Decode nevent`` () =
       // should equal "nevent1qqsq3gvnfyk8lvn6k8v47fvyv8j2ph78754ue40qyf6xev5yrrh5jpgppamhxue69uhkummnw3ezumt0d5pzp75cf0tahv5z7plpdeaws7ex52nmnwgtwfr2g3m37r844evqrr6jqvzqqqqqqycnq9s3" encodedNevent 
        NEvent(
            ...
            ["wss://nostr.mom"],
            Some (ECXOnlyPubKey.Create(Utils.fromHex "fa984bd7dbb282f07e16e7ae87b26a2a7b9b90b7246a44771f0cf5ae58018f52")),
            Some 0
            )
        |> encodeDecode
        |> function
            | NEvent(EventId eventId, ["wss://nostr.mom"], Some ecxOnlyPubKeyOption, Some 0) ->
